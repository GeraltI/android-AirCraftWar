    ...
    public List<BaseBullet> shootStrategy(int shootNum, int locationX, int locationY, int speedX, int speedY, int power){
        ...
        if (shootNum % 2 == 1) {
            for(int i=0; i<shootNum; i++){
                // 子弹发射位置相对飞机位置向前偏移
                // 多个子弹横向分散
                baseBullet = new EnemyBullet(locationX, locationY, (2 * i - shootNum + 1)/2, 12, power);
                res.add(baseBullet);
            }
        }
        else{
            for(int i=0; i<shootNum; i++){
                ...
                if(i < shootNum / 2)
                {
                    baseBullet = new EnemyBullet(locationX, locationY, (2 * i - shootNum)/2, 12, power);
                    res.add(baseBullet);
                }
                else{
                    baseBullet = new EnemyBullet(locationX, locationY, (2 * i - shootNum + 2)/2, 12, power);
                    res.add(baseBullet);
                }
            }
        }
